...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Load and attach prediction toggle state
  const { predictiveToggle } = await chrome.storage.local.get({ predictiveToggle: true });
  const toggleInput = document.getElementById('predictiveToggle');
  if (toggleInput) {
    toggleInput.checked = predictiveToggle;
    toggleInput.addEventListener('change', async () => {
      await chrome.storage.local.set({ predictiveToggle: toggleInput.checked });
      const status = document.getElementById('status');
      status.textContent = "Prediction settings saved!";
